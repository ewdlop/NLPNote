...
# Cross-Platform Build Script for NLP Note
...
mkdir -p dist/win-x64 dist/win-arm64 dist/linux-x64 dist/linux-arm64 dist/osx-x64 dist/osx-arm64 dist/android dist/harmonyos
...
# Build Console Application for different platforms
...
# Android build (if Android SDK is available)
echo "Attempting Android build..."
if command_exists android && [ -n "$ANDROID_HOME" ]; then
...
if [ -d "dist/android" ] && [ "$(ls -A dist/android)" ]; then
...
echo "Windows: ./dist/win-x64/nlpnote.exe [command]"
